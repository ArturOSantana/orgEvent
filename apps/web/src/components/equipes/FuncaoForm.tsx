...
import { criarFuncao } from '../../services/equipeService'
...
interface FuncaoFormProps {
  eventoId: string
  onSuccess: (funcao: Funcao) => void
  onCancel: () => void
}

export function FuncaoForm({ eventoId, onSuccess, onCancel }: FuncaoFormProps) {
  const {
    register,
    handleSubmit,
  ...
  async function onSubmit(values: FormValues) {
    try {
      const resultado = await criarFuncao(eventoId, {
        nome: values.nome,
        descricao: values.descricao || undefined,
      })
      onSuccess(resultado)
    } catch {
      alert('Ocorreu um erro ao salvar a funcao. Tente novamente.')
    }
  }

  return (
    <Modal open onClose={onCancel} title="Nova Funcao">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        <Input
          label="Nome *"
          placeholder="Ex: Lider de grupo, Motorista..."
